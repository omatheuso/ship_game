defmodule ShipWeb.GameLive do
  ...
  def mount(_params, %{"player_token" => token} = _session, socket) do
    ...
    socket =
      ...
      |> assign(game_world_size: 300, screen_height: 50, screen_width: 50)
      ...
      >
        ...
        <% else %>
          ...
          <%= for {_entity, x, y, image_file} <- @other_ships do %>
            <image x={x} y={y} width="2" height="2" href={~p"/images/#{image_file}"} />
          <% end %>
          <text x={@x_offset + 1} y={@y_offset + 2} style="font: 2px serif">
